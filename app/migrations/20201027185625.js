...
const template = {
  name: 'Quote Template PTS_LMS-95_CAD_01',
  type: 'Quote',
  template: `
<table>
    <tbody>
...
<div class="divTable auto-width-table">
    <div class="divTableBody">
        ...
        <div class="divTableRow">
            ...
            <div class="divTableCell">{{toFixed invoice.foreignUnitPrice 5}}</div>
            <div class="divTableCell text-right">{{requestCurrency}} {{toFixed invoice.foreignTotal 5 }}<br /></div>
        </div>
        ...
        <div class="divTableRow">
            ...
            <div class="divTableCell"></div>
            {{#if invoice.workflow.last}}
            <div class="divTableCell text-right"><b>Subtotal</b> {{requestCurrency}} {{ toFixed invoice.workflow.foreignSubtotal 5 }}</div>
            {{ else }}
            <div class="divTableCell text-right">{{requestCurrency}} {{ toFixed invoice.workflow.foreignSubtotal 5 }}</div>
            {{/if}}
        </div>
        {{/if}} {{/each}}
    </div>
...
<div class="total text-right float-right">
    <div class="blue-background font-weight-bold">Total {{requestCurrency}} {{ toFixed request.foreignInvoiceTotal 5 }}</div>
</div>
<h6><br /></h6>
...
const migration = () => mongo.connect(configuration)
  ...
  .then((db) => {
    ...
    return lspCol.findOne({ name: 'Protranslating' })
      .then(lsp =>
        templatesCol.findOne({ name: 'Quote Template PTS_LMS-95_CAD_01', lspId: lsp._id })
          .then((dbTemplate) => {
