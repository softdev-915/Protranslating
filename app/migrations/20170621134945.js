...
const requestCreateEmail = {
  ...
  email: {
    ...
    template: '<p>Request details:</p><p>Number : {{request.no}}</p><p>Documents: {{allDocumentsNames request.documents}}</p><p>Title: {{request.title}}</p>',
...
const requestProjectManagerEmail = {
  ...
  email: {
    from: 'support@protranslating.com',
    template: '<p><strong>Customer Name:</strong> {{request.customer.name}}</p><p><strong>Requested By:</strong> {{username request.contact}}</p><p><strong>Request Number:</strong> {{request._id}}</p><p><strong>Document(s) Name:</strong> {{#each request.documents}}{{name}},{{/each}}</p><p><strong>Reference Document(s) Name:</strong> {{#each request.documents}}{{name}},{{/each}}</p><p><strong>Requested Delivery Date:</strong> {{toTimezoneOffset request.deliveryDate -5 \'YYYY-MM-dd hh:mm A Z\'}}</p><p><a href=href="{{path}}requests/{{request._id}}/details" target="_blank">Go to this Request</a></p>',
...
const quotedRequestProjectManagerEmail = {
  ...
  email: {
    from: 'support@protranslating.com',
    template: '<p><strong>Request Number:</strong> {{request._id}}</p><p><strong>Customer:</strong> {{request.customer.name}}</p><p><strong>Requested by:</strong> {{username request.contact}}</p><p><strong>Note from contact:</strong> {{request.comments}}</p><p><a href=href="{{path}}requests/{{request._id}}/details" target="_blank">Go to this Request</a></p>',
...
const requestModifiedProjectManagerEmail = {
  ...
  email: {
    from: 'support@protranslating.com',
    template: '<p>Hello {{username user}}, </p><p>Modified Request</p><p>A request has been modified. Please kindly note the changes which have been made as they may concern you.</p><p>Customer Name: {{request.customer.name}}<p>Requested By: {{username request.contact}}</p><p>Request Number: {{request._id}}</p><p>Modifications: <table><thead><tr> <th>Element(s)</th> <th>Old Value</th> <th>New Value</th></tr></thead><tbody>{{#each modifications}}<tr><td>{{name}}</td><td>{{oldValue}}</td><td>{{value}}</td></tr>{{/each}}</tbody></p><p>Delivery Date:{{toTimezoneOffset request.deliveryDate -5 \'YYYY-MM-dd hh:mm A Z\'}}</p><p><a href=href="{{path}}requests/{{request._id}}/details">Go to this Request</a></p>',
...
const migration = () => mongo.connect(configuration)
  ...
  .then((db) => {
    ...
    return insertIfMissing(schedulers, requestCreateEmail)
      .then(() => insertIfMissing(schedulers, requestProjectManagerEmail))
