...
const activityFeedbackUpdateEmail = {
  name: 'user-feedback-update-for-auditor',
  lspId: 'lsp ID',
  every: '1 minutes',
  options: {
    lockLifetime: 10000,
  ...
  email: {
    ...
    <p><strong><ins>FEEDBACK REVIEW UPDATE</ins></strong><p>
    An Activity-Feedback record has been updated. Please review the information below:<br>
      ...
      .then((lsps) => {
        if (lsps && lsps.length) {
          return Promise.all(lsps.map((lsp) => {
            const schedulerToInsert = Object.assign({}, activityFeedbackUpdateEmail);
