...
const _ratesAreDuplicates = (a, b) => {
  ...
  const fieldsAreIdentical = toCompare.every((field) => {
    const fieldA = _.isObject(toCompareA) ? _.get(toCompareA, `${field}.name`) : toCompareA[field];
    const fieldB = _.isObject(toCompareB) ? _.get(toCompareB, `${field}.name`) : toCompareB[field];
...
/**
 * Returns true if rate is a duplicate of rates list
 * @param {Object} rateToCompare object
 * @param {Object} rates list to compare with rateToCompare
 * @returns {Boolean}
 */
module.exports.rateIsDuplicate = (rateToCompare, rates) => {
  const rateHasDuplicatedDetails = _hasIdenticalDetails(rateToCompare.rateDetails);
  if (rateHasDuplicatedDetails) return true;
  const hasRatesErrors = rates.some(rate => _ratesAreDuplicates(rateToCompare, rate));
