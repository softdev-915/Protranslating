<template>
  <div class="ip-wrapper">
    <div class="ip-card wizard-ip-card">
      ...
      <div class="ip-card__body">
        <div v-if="currentStep === 0">
          ...
          <div class="counts-wrapper">
            ...
            <ip-input
              ...
              v-model="drawingsPageCount"
            />
          </div>
          <h4 v-if="isNew" class="body__header mt-3">File(s)</h4>
          <ip-file-upload v-if="isNew" @file-upload="onFilesUpload" :required="isRequired && files.length === 0" class="mt-4" data-e2e-type="source-files"/>
        </div>
        ...
        <div class="instant-quote-container" v-else-if="currentStep === 2">
          ...
          <template v-if="showTranslationFees">
            <table
              v-if="calculatedCountries"
              class="instant-quote-table-translation-only preview-table"
              data-e2e-type="instant-quote-table"
            >
              <thead>
                <tr>
                  <th class="country-row">COUNTRY</th>
                  <th class="translation-fee right">TRANSLATION FEE</th>
                </tr>
              </thead>
              <tbody>
                <tr
                  v-for="(country, index) in translationFees"
                  :key="index"
                >
                  <td>{{ country.country }}</td>
                  <td v-if="!country.neededQuotation" class="right">
                    <span class="currency-symbol">{{ quoteCurrency.symbol }} </span>
                    <span>{{
                      formatFee(
                        country.translationFeeCalculated[quoteCurrency.isoCode]
                      )
                    }}</span>
                  </td>
                  <td v-else class="custom-quote right">
            ...
            <div class="body__price">
              ...
              <div class="price__total">
                <span>TOTAL: </span>
                <span data-e2e-type="total-price">{{
                  `${quoteCurrency.isoCode} ${formatFee(totalFee)}`
                }}</span>
              </div>
            </div>
            <div class="body__bullets mt-4" data-e2e-type="quote-disclaimers">
              <p
                v-for="(disclaimer, index) in quoteDisclaimers"
                :key="`index-deisclaimer-${index}`"
              >
                {{ `• ${disclaimer}` }}
              </p>
            </div>
    ...
    <ip-card class="ml-3 quote-details-card" data-e2e-type="quote-details-card">
      ...
      <div class="ip-card__detail">
        ...
        <template v-if="isFirstStepFilled">
          ...
          <div class="detail__info mt-3">
            ...
            <div class="record d-flex flex-column align-items-start">
              <span class="record__header">Number of Drawings Pages</span>
              <span>{{ formatNumber(drawingsPageCount) }}</span>
            </div>
          </div>
        ...
        <template v-if="isThirdStepFilled">
          ...
          <div class="detail__info mt-3">
            <div class="record-wide d-flex flex-column align-items-start" data-e2e-type="country-details">
        ...
        <div class="ip-modal-body">
          ...
          <div class="ip-modal-body__controls">
            <ip-button class="mr-4" @click.native="closeModal" data-e2e-type="discard-cancel"
              >Cancel</ip-button
            >
            <ip-button type="filled" @click.native="discardQuote" data-e2e-type="discard-approve">Discard</ip-button>
          </div>
        </div>
      </template>
    </ip-modal>
    <ip-modal width="600px" v-model="showApproveModal">
      ...
      <div class="ip-modal-body">
        ...
        <ip-input
          ...
          data-e2e-type="approve-comment"
        />
        <div class="ip-modal-body__subheader mt-4">Files</div>
        <ip-file-upload data-e2e-type="approve-files" @file-upload="onFilesUploadApprove" class="mt-4" />
        <div class="ip-modal-body__controls">
          <ip-button data-e2e-type="approve-cancel" class="mr-4" @click.native="showApproveModal = false"
            >Cancel</ip-button
          >
          <ip-button data-e2e-type="approve-accept" type="filled" @click.native="saveAndSeeQuote"
...
<style scoped lang="scss" src="./nodb.scss"></style>
