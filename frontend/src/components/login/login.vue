<template>
  <div id="login" data-e2e-type="login-container" class="row justify-content-center pts-login-main" :class="{'blur-loading-container': loading}">
      ...
      <div class="container">
        ...
        <div class="row justify-content-center">
          <div class="col-md-8 col-sm-12 main-form-container mb-4">
            <form novalidate @submit="submitEmail($event)" v-if="!isPasswordAccepted && !emailSubmitted">
            ...
            <form novalidate @submit="submitPassword($event)" v-else-if="!isPasswordAccepted&&emailSubmitted">
              <div class="email-entered mb-2 center" v-show="!submitting" :class="{'blur-loading': submitting}">
                <span @click="reset"><i class="fas fa-arrow-left"></i></span>
                <span @click="reset" class="email-box">{{email}}</span>
              </div>
              <div class="lsp pb-4 center" v-show="userLspList.length > 1" :class="{'blur-loading': loading}">
                <label for="lspSelector" class="sr-only">Lsp</label>
                <login-lsp-selector
                  id="lspSelector"
                  data-e2e-type="lsp-selector"
                  autocomplete="no"
                  :lsp-list="userLspList"
                  v-model="lsp">
                </login-lsp-selector>
              </div>
              ...
              <div class="text-center captcha-container mb-4" v-if="isCaptchaVisible" :class="{'blur-loading': loading}">
                <re-captcha-v3
                  ref="reCaptcha"
                  :site-key="siteKey"
                  :mock="!isProductionEnvironment"
                  @re-captcha-solved="onRecaptchaValidation"
                  @re-captcha-error="onRecaptchaError"
                  @re-captcha-loading="onRecaptchaLoading">
                </re-captcha-v3>
              </div>
            </form>
            <form class="mb-5" @submit="submitHotp" v-else>
        ...
        <div class="row justify-content-center">
          <template v-if="!emailSubmitted">
            <div class="col-12 text-center mb-4">
              <span class="info-text">By clicking GO I am agreeing with BIG Language Solution's <a data-e2e-type="login-tc-link" rel="noopener noreferrer" href="https://biglanguage.com/privacy/" target="_blank">Terms &amp; Conditions</a> and <a data-e2e-type="login-pp-link" rel="noopener noreferrer" href="https://biglanguage.com/privacy/" target="_blank">Privacy Policy</a></span>
            </div>
